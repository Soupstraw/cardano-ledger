...
import Cardano.Ledger.SafeHash (SafeToHash(..), SafeHash)
...
import Data.Coders (Encode, runE, encode)
...
pattern Memo mt mb <- Memo' mt mb _
...
instance (Typeable t, Typeable era) => ToCBOR (MemoBytes t era) where
  toCBOR (Memo' _ bytes _) = encodePreEncoded (fromShort bytes)

instance (Era era, Typeable t, Typeable era, FromCBOR (Annotator (t era))) => FromCBOR (Annotator (MemoBytes t era)) where
  fromCBOR = do
...
roundTripMemo :: (Era era, FromCBOR (t era)) => MemoBytes t era -> Either Codec.CBOR.Read.DeserialiseFailure (Lazy.ByteString, MemoBytes t era)
roundTripMemo (Memo' _t bytes _) =
  case deserialiseFromBytes fromCBOR (fromStrict (fromShort bytes)) of
    Left err -> Left err
    Right (leftover, newt) -> Right (leftover, mkMemoBytes newt (LBS.fromStrict $ fromShort bytes))

-- | Create MemoBytes from its CBOR encoding
memoBytesFromEncoding :: Era era => Encode w (t era) -> MemoBytes t era
memoBytesFromEncoding t = mkMemoBytes (runE t) (toLazyByteString (encode t))
