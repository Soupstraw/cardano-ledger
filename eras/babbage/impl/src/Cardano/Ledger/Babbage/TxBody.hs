...
module Cardano.Ledger.Babbage.TxBody
  ( BabbageTxOut (BabbageTxOut, TxOutCompact, TxOutCompactDH, TxOutCompactDatum, TxOutCompactRefScript),
    AlonzoEraTxOut(..),
    BabbageEraTxOut (..),
    BabbageTxBody
      ( BabbageTxBody,
...
class AlonzoEraTxOut era => BabbageEraTxOut era where
...
lensTxBodyRaw ::
  (EraTxBody era, ToCBOR (PParamsUpdate era), ToCBOR (Script era)) =>
  (TxBodyRaw era -> a) ->
...
pattern BabbageTxBody ::
  (EraTxOut era, ToCBOR (PParamsUpdate era), ToCBOR (Script era)) =>
  Set (TxIn (Crypto era)) ->
  Set (TxIn (Crypto era)) ->
  Set (TxIn (Crypto era)) ->
...
decodeTxOut = do
  dtxo <- decode $ SparseKeyed "BabbageTxOut" initial bodyFields requiredFields
  case dtxo of
